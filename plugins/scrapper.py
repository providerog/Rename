...
async def optimized_batch_rename_async(mega_session, batch_files, prefix, user_id, progress_callback=None):
    ...
    if progress_callback:
        await progress_callback(len(batch_files), len(batch_files), results)
...
async def handle_mega_folder_processing_async(mega_session, message, prefix, user_id):
    ...
    try:
        ...
        # Process in optimal batches with async processing
        batch_size = 50  # Smaller batches for better responsiveness

        # Progress callback function
        async def progress_callback(processed, total_batch, batch_results):
            nonlocal renamed_count, failed_count, skipped_count

            # Update counts from batch results
            for result_type, old_name, new_name in batch_results:
                if result_type == 'success':
                    logger.info(f"✅ Renamed: {old_name} → {new_name}")
                elif result_type == 'skipped':
                    pass
                else:
                    logger.debug(f"❌ Failed: {old_name}")

        for i in range(0, len(media_files_to_process), batch_size):
            # Check if operation was cancelled
            if not active_operations.get(user_id, {}).get('active', True):
                await status.edit("❌ **ᴏᴘᴇʀᴀᴛɪᴏɴ ᴄᴀɴᴄᴇʟʟᴇᴅ ʙʏ ᴜsᴇʀ!**")
                return

            batch = media_files_to_process[i:i + batch_size]

            # Process batch asynchronously
            results = await optimized_batch_rename_async(
                mega_session, batch, prefix, user_id, progress_callback
            )
            ...
            try:
                ...
                processed_files = renamed_count + failed_count + skipped_count
                ...
                progress_percent = (processed_files / total_media_files) * 100

                await status.edit(
                    f"🚀 **ᴘʀᴏᴄᴇssɪɴɢ... ({processed_files}/{total_media_files})**\n\n"
                    f"✅ **ʀᴇɴᴀᴍᴇᴅ:** {renamed_count}\n"
